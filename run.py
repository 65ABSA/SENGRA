...
def train_and_evaluate(model_func, args, save_specific=False):
    ...
    vocab['deprel_vocab'] = Vocab.load_vocab(dataset_dir + "/vocab_deprel.vocab")  # deprel
    print(vocab['deprel_vocab'].stoi)
    print(vocab['deprel_vocab'].itos)

    class_n = len(label2id)
    args.class_n = class_n
    dep_n = len(vocab['deprel_vocab'])
    args.dep_n = dep_n
    ...
    base_model = model_func(pretrained_model_path = args.pretrained_model,
                                ...
                                dep_n=args.dep_n).to(args.device)
...
def get_parameters():
    ...
    parser.add_argument("--dep_dim", type=int, default=40)
