...
class ASTE_End2End_Dataset(Dataset):
    def __init__(self, file_name, vocab = None, version = '3D', tokenizer = None, max_len = 128, lower=True, is_clean = True):
        ...
        json_file_name = os.path.splitext(file_name)[0].replace('_triplets', '') + ".json"
        
        if type(file_name) is str:
            with open(file_name,'r',encoding='utf-8') as f:
                lines = f.readlines()
                data_base = [line2dict(l, is_clean=is_clean) for l in lines]
            with open(json_file_name, 'r', encoding='utf-8') as f:
                data_addon = json.load(f)
            self.raw_data = merge_data(data_base, data_addon)
        else:
    ...
    def preprocess(self, data, vocab,version):
        ...
        for d in data:
            ...
            dep = d['deprel']
            dep = dep[:length]
            dep_vocab = vocab['deprel_vocab']
            dep = [dep_vocab.stoi.get(t, dep_vocab.unk_index) for t in dep]

            dep_head = d['head']
            dep_head = dep_head[:length]
...
def ASTE_collate_fn(batch):
    ...
    deprel_head = get_long_tensor([batch[i]['deprel_head'] for i in range(batch_size)])
    deprel = get_long_tensor([batch[i]['deprel'] for i in range(batch_size)])
...
def merge_data(data1, data2):
    """
    将 data2 的 postag、head 和 deprel 合并到 data1 中。
    :param data1: 第一个数据源（包含 token 和 triplets）。
    :param data2: 第二个数据源（包含 postag、head 和 deprel 等）。
    :return: 合并后的数据。
    """
    merged_data = []

    # 遍历 data1 和 data2 进行匹配
    for item1, item2 in zip(data1, data2):
        merged_item = item1.copy()  # 复制 data1 中的条目
        # 从 data2 中提取所需的 postag, head, 和 deprel
        # merged_item['postag'] = item2.get('postag', [])
        merged_item['head'] = item2.get('head', [])
        merged_item['deprel'] = item2.get('deprel', [])
        #print(merged_item['token'],len(merged_item['token']))
        #print(merged_item['deprel'],len(merged_item['deprel']))
        assert len(merged_item['token']) == len(merged_item['deprel'])
...
def print_nested(data, indent=0):
    ...
    if isinstance(data, list):
        ...
        print(f"{spacing}{repr(data)}")  # 打印基本类型值