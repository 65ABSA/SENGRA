...
import torch
from scheme.sublayer import PositionwiseFeedForward, MultiHeadedAttention


class RGATLayer(nn.Module):
    _forward_counter = 0  # 类变量，用于全局计数
    def __init__(
        self, d_model, heads, d_ff, dropout, att_drop=0.1, use_structure=True, dep_dim=30, alpha=1.0, beta=1.0, use_gating=True
    ):
        super(RGATLayer, self).__init__()
        self.self_attn = MultiHeadedAttention(
            heads, d_model, dropout=att_drop, use_structure=use_structure, structure_dim=dep_dim, alpha=alpha, beta=beta, use_gating=use_gating
        )
    ...
    """
        ...
        """
        RGATLayer._forward_counter += 1
        if RGATLayer._forward_counter == 124:
            print(f"\n=== RGAT Forward #{RGATLayer._forward_counter} ===")
            print(f"attn shape: {attn.shape}")
            print(attn[0])  # 打印第一个样本的注意力矩阵（可视化用）
        """
