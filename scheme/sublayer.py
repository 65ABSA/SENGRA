...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, structure_dim=30, dropout=0.1, use_structure=False, alpha=1.0, beta=1.0, use_gating=True):
        ...
        self.use_gating = use_gating # <--- 保存标志位
        ...
        if self.use_gating:
            self.gate_linear = nn.Linear(model_dim, head_count * self.dim_per_head)
    ...
    def forward(
        ...
        if self.use_gating:
            gate_score = self.gate_linear(input_for_gate) # -> [B, Seq, Head * Dim]
            gate_score = shape(gate_score)
            gate_score = torch.sigmoid(gate_score)
            context = context * gate_score
        ...
        if structure_k is not None:
            print("structure_k mean:", structure_k.mean().item(), "std:", structure_k.std().item())
